...
def SituationalPainScale(outputPath, subjectNum, questPath):
    ...
    x = pd.read_csv('SituationalPainQuestionnaire.csv')
    y = [x.T.to_dict()]
    
    #survey = Form.Form(mywin, items=y, size=(1.0, 0.7), pos=(0.0, 0.0))
    
    Instructions = visual.TextStim(win=mywin, text=("Imaginez l'intensité de la douleur que vous pourriez ressentir dans chacune des situations décrites ci-dessous - Evaluez cette intensité avec l'échelle des sensations (partie droite de l'écran)."),
    font='', height = 0.04, pos=(0, 0.85), units = 'norm', wrapWidth = 2)
    
    survey = Form.Form(mywin, items='SituationalPainQuestionnaire.csv', size=(1.0, 0.7), pos=(0.0, 0.0),  autoLog=True)
    ...
    os.chdir(questPath)
    Questions.to_csv(subjectNum + 'SPS_Scale_Response.csv')
    mywin.close()
